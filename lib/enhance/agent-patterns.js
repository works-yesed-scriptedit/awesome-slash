...
const agentPatterns = {
  ...
  /**
   * Hardcoded CLAUDE_PLUGIN_ROOT
   * MEDIUM certainty - needs adapter substitution
   */
  hardcoded_plugin_root: {
    id: 'hardcoded_plugin_root',
    category: 'cross-platform',
    certainty: 'MEDIUM',
    autoFix: false,
    description: 'Uses ${CLAUDE_PLUGIN_ROOT} without adapter awareness',
    check: (content) => {
      if (!content || typeof content !== 'string') return null;

      const usesClaudeRoot = /\$\{CLAUDE_PLUGIN_ROOT\}/.test(content);

      if (usesClaudeRoot) {
        return {
          issue: 'Uses ${CLAUDE_PLUGIN_ROOT} - requires adapter path substitution',
          fix: 'Ensure adapters/*/install.sh handles path substitution for this file'
        };
      }
      return null;
    }
  ...
  claude_md_reference: {
    ...
    certainty: 'MEDIUM',
    autoFix: false,
    description: 'References CLAUDE.md (OpenCode/Codex use AGENTS.md)',
    check: (content) => {
      if (!content || typeof content !== 'string') return null;
      ...
      if (hasClaudeMd) {
        return {
          issue: 'References CLAUDE.md - platform-specific',
          fix: 'Use generic term like "project instructions" or check for both CLAUDE.md and AGENTS.md'
        };
      }
      return null;
    }
